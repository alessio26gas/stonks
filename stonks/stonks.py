...
def main():
    ...
    try:
        ...
        for symbol in symbols:
            symbols[symbol] = float(symbols[symbol])
    ...
    while 1:
        try:
            ...
            futures = [
                executor.submit(get_stock_data, symbol, symbols[symbol])
                for symbol in symbols
            ]
            list = [
            ...
            if delay != 0 and (debugtime1 - start) > 0.1:
                sleep(delay - (time() - start) % delay)
            cls()
            print("\033[1mSTONKS\033[0m v1.0.0")
            ...
            print(
                ...
                + (
                    (
                        ...
                        + "P&L($)".rjust(10)
                        + (
                            ("P&L(€)" if curr == "EUR" else f"P&L({curr})")
                            if curr != "USD"
                            else ""
                        ).rjust(10)
                    )
            ...
            for ticker in list:
                ...
                print(
                    color(ticker.change)
                    + f"{ticker.name}".ljust(7)
                    + f"{ticker.volume}".rjust(8)
                    + ("  ▲ " if ticker.change >= 0 else "  ▼ ")
                    + f"{ticker.price:.2f}".rjust(7)
                    + f"{ticker.change:+.2f}".rjust(8)
                    + f"({ticker.percent_change:+.2f}%)".rjust(10)
                    + (
                        (
                            f"{ticker.shares:g}".rjust(8)
                            + f"{ticker.shares * ticker.change:+.2f}".rjust(10)
                            + (
                                (
                                    f"{ticker.shares * ticker.change * currency.price:+.2f}".rjust(
                                        10
                                    )
                                )
                                if curr != "USD"
            ...
            if curr != "USD":
                ...
                print(
                    color(currency.change)
                    + f"{currency.name}".ljust(10)
                    + ("▲ " if currency.change >= 0 else "▼ ")
                    + f"{currency.price:.5f}".rjust(7)
                    + f"{currency.change:+.5f}".rjust(10)
                    + f"({currency.percent_change:+.2f}%)".rjust(10)
                    + "\033[0m"
                )
                ...
                if not noshares:
                    print(
                        ...
                        + color(profit)
                        + (
                            ((f"€") if curr == "EUR" else f"{curr} ")
                            + f"{total * currency.price:.2f}"
                        ).rjust(10)
                        + f"{profit * currency.price:+.2f}".rjust(10)
                        + f"({100 * profit / (total - profit):+.2f}%)".rjust(10)
                        + "\033[0m"
                    )
...
def createconfig():
    config = ConfigParser()
    config["Portfolio"] = {"AAPL": 10, "GOOG": 10, "AMZN": 0, "MSFT": 0}
    config["Settings"] = {
        "currency": "USD",
        "refresh-delay": 1.0,
    ...
    with open(".stonks.cfg", "w") as f:
        ...
        f.write(
            "## Type tickers in your portfolio specifying the number of shares you own (es. 'AAPL=30')\n## or add them to your watchlist using 0 (es. 'AMZN=0')\n"
        )
        config.write(f)
